module gnssrspd_setup
  implicit none
  private
  public:: setup
        interface setup; module procedure setupgnssrspd; end interface

contains
subroutine setupgnssrspd(obsLL,odiagLL,lunin,mype,bwork,awork,nele,nobs,is,conv_diagsave)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    setupgnssrspd    compute rhs of oi for wind speed obs
!   prgmmr: Karina Apodaca org: Spire Global, Inc.    date: 2023-03-15
!   Based on: setup10mspd.f90 by:  
...
!$$$
  ...
  use m_gnssrspdNode, only: gnssrspdNode_appendto
...
! Declare local variables
  ...
  real(r_kind) uob,vob,gnssrspdges,gnssrspdob,gnssrspdob0,goverrd,ratio_errors
  ...
  type(gnssrspdNode),pointer:: my_head
  ...
  type(obsLList),pointer,dimension(:):: gnssrspdhead
  gnssrspdhead => obsLL(:)
...
!    Load obs error and u,v obs
     ...
     gnssrspdob=sqrt(uob*uob+vob*vob)
...
! guess wind speed.
     ...
     ddiff = gnssrspdob-gnssrspdges

!    write(6,*) "cygnss gnssrspdob=",gnssrspdob," gnssrspdges=",gnssrspdges," ddiff=",ddiff
...
! as a function of observation type.
     ...
     if (luse_obsdiag) then
        call obsdiagNode_set(my_diag, wgtjo=(error*ratio_errors)**2, &
           jiter=jiter, muse=muse(i), nldepart=gnssrspdob-sqrt(ugesin*ugesin+vgesin*vgesin))
     endif
...
!    in inner loop minimization (int* and stp* routines)
     if (.not. last .and. muse(i)) then
        ...
        allocate(my_head)
        call gnssrspdNode_appendto(my_head,gnssrspdhead(ibin))
...
! Save select output for diagnostic file
     if(conv_diagsave .and. luse(i))then
        ii=ii+1
        gnssrspdob0      = sqrt(data(iuob,i)*data(iuob,i)+data(ivob,i)*data(ivob,i))
...
900  format('conv_gnssrspd_',i2.2,'.nc4')
  ...
  type(obs_diag),pointer,intent(in):: odiag
        ...
        rdiagbuf(17,ii) = gnssrspdob              ! wind speed observation (m/s)
        rdiagbuf(18,ii) = ddiff              ! obs-ges used in analysis (m/s)
        rdiagbuf(19,ii) = gnssrspdob0-gnssrspdges      ! obs-ges w/o bias correction (m/s) (future slot)
  ...
  type(obs_diag),pointer,intent(in):: odiag
! Observation class
  character(8),parameter     :: obsclass = 'gnssrspd'
...
end subroutine setupgnssrspd
end module gnssrspd_setup
